...
# ==========================================
# 🔒 پنل ادمین برای دانلود دیتا
# ==========================================
st.sidebar.markdown("---")
st.sidebar.markdown("🛡️ **پنل مدیریت (فقط برای پژوهشگر)**")

# دریافت رمز از کاربر
admin_password = st.sidebar.text_input("رمز عبور ادمین:", type="password")

# امن‌ترین روش خواندن رمز (بدون هیچ‌گونه رمز پیش‌فرض و قابل حدس)
CORRECT_PASSWORD = st.secrets.get("ADMIN_PASSWORD", None)

if admin_password:
    if CORRECT_PASSWORD is None:
        # اگر رمز در استریم‌لیت ست نشده باشد، سیستم قفل می‌شود
        st.sidebar.error("⚠️ خطای سرور: رمز عبور در بخش Secrets تنظیم نشده است. سیستم قفل شد.")
    elif admin_password == CORRECT_PASSWORD:
        st.sidebar.success("✅ دسترسی تایید شد")
        
        # ۱. دکمه دانلود فایل اکسل (نتایج ارزیابی انسانی)
        if os.path.exists(CSV_FILE) and os.path.getsize(CSV_FILE) > 0:
            with open(CSV_FILE, "rb") as file:
                st.sidebar.download_button(
                    label="📥 دانلود نتایج ارزیابی (Excel)",
                    data=file,
                    file_name="human_evaluation_results.csv",
                    mime="text/csv",
                    type="primary"
                )
        else:
        ...
        if os.path.exists(DB_FILE):
            with open(DB_FILE, "rb") as db_file:
                st.sidebar.download_button(
                    label="🧠 دانلود دیتابیس حافظه طلایی",
                    data=db_file,
                    file_name="golden_memory_backup.db",
